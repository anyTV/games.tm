...
if(isset($_GET))
{
  ...
  // echo 'get:' . $_GET['g'];
  // echo 'url:' . $url;
  if(empty($resultarray->response->data->data)){ 
    echo 'false';
    return ;
  }
  ...
  // echo '<pre>' . print_r($queryId, true) . '</pre>';
  $duplicate = $vidstats;
  foreach ( $resultarray->response->data->data as $stat){
    $url = parse_yturl($stat->Stat->url);
    if($url){
      foreach ($queryId as $key => $value) {
        if($queryId[$key]==$url){
           ...
           // else{
           //    $vidstats[$key-1]['Sources']['clicks'] += $stat->Stat->clicks;
           // }
        }
      }
    }
  }
 // echo '<pre>' . print_r($vidstats, true) . '</pre>';
  $temporary = array();
